...
class Roleplay():    
    ...
    def Tiny(self):
        ...
        role = random.choice([animal[0][0], animal[1][0], animal[2][0]])
        
        cm.tts(bhv="do_suggestion_S", string=f"이제 우리는 작은 {wm.word(role, 0)}야!")
        
        cm.tts(bhv="do_suggestion_L", string=f"{wm.word(self.user_name, 0)}도 {role} 소리를 흉내내 보자! 시작!")
        answer = cm.responses_proc(re_bhv="do_suggestion_L", re_q=f"{wm.word(self.user_name, 0)}도 {role} 울음 소리를 흉내내보자! 시작!",
                                   neu_bhv="do_explain_A", neu=f"{wm.word(role, 2)} 이렇게 울어~",
                                   act_bhv="do_joy_A", act=f"{wm.word(role, 1)} 나타났다!")
        # audio.audio_play(filename)
        cm.tts(bhv="do_explain_B", string=animal[rand][2])
        ...
        while True:
            for i in range(len(rand)):
                if rand[i] == 1:                             
                    cm.tts(bhv="do_question_L", string=f"{wm.word(self.user_name, 0)}가 {wm.word(role, 1)} 된다면 어떤 점이 가장 좋을까?")
                    answer = cm.responses_proc(re_bhv="do_question_L", re_q=f"{wm.word(self.user_name, 0)}가 {wm.word(role, 1)} 된다면  어떤 점이 가장 좋을까?",
                ...
                if rand[i] == 2: 
                    ...
                    if answer[0] == "positive" or "neutral":
                ...
                if rand[i] == 3: 
                    cm.tts(bhv="do_question_L", string=f"{wm.word(self.user_name, 0)}가 {wm.word(role, 2)} 되어서 숨고 싶다고 느낀 적이 있니?")
                    answer = cm.responses_proc(re_bhv="do_question_L", re_q=f"{wm.word(self.user_name, 0)}가 {wm.word(role, 2)} 되어서 숨고 싶다고 느낀 적이 있니?",
                ...
                if rand[i] == 4: 
                    cm.tts(bhv="do_question_L", string=f"{wm.word(self.user_name, 0)}가 {wm.word(role, 1)} 된다면 어디에 살고 싶니?")
                    answer = cm.responses_proc(re_bhv="do_question_L", re_q=f"{wm.word(self.user_name, 0)}가 {wm.word(role, 2)} 된다면 어디에 살고 싶니?",
                                               pos_bhv="do_question_S", pos="언제 숨고 싶었니?",
                ...
                if rand[i] == 5: 
                    cm.tts(bhv="do_question_L", string=f"{wm.word(self.user_name, 0)}는 {wm.word(role, 2)} 되어서 몰래 가보고 싶은 곳이 있니?")
                    answer = cm.responses_proc(re_bhv="do_question_L", re_q=f"{wm.word(self.user_name, 0)}가 {wm.word(role, 2)} 된다면 어디에 살고 싶니?",
                ...
                if rand[i] == 6: 
                    cm.tts(bhv="do_question_L", string=f"{wm.word(self.user_name, 0)}는 {wm.word(role, 2)} 되어서 몰래 도와주고 싶은 사람이 있니?")
                    answer = cm.responses_proc(re_bhv="do_question_L", re_q=f"{wm.word(self.user_name, 0)}는 {wm.word(role, 2)} 되어서 몰래 도와주고 싶은 사람이 있니?",
        ...
        # 4. 마무리 대화
        cm.tts(bhv="do_question_L", string=f"{wm.word(role, 2)} 작지만 멋진 친구들이야! 다음에 또 재미있는 역할놀이 하자~")
