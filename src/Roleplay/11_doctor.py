...
class Roleplay():    
    ...
    def Doctor(self):      # 이 시나리오 미완성인듯
        ...
        cm.tts(bhv="do_suggestion_S", string=f"오늘은 의사 역할 놀이를 해볼거야~") 
                
        # 2. 역할 놀이
        cm.tts(bhv="do_question_L", string=f"{wm.word(self.user_name, 0)}는 언제 병원에 가니?")        
        answer = cm.responses_proc(re_bhv="do_question_L", re_q=f"{wm.word(self.user_name, 0)}는 언제 병원에 가니?",
                                   neu_bhv="do_agree", neu="몰라도 괜찮아~")
        
        cm.tts(bhv="do_explain_A", string=f"병원은 아픈 사람들을 치료해 주는 곳이야. {wm.word(self.user_name, 0)}가 의사 선생님이 되어서 나를 치료해 줘~")
        
        cm.tts(bhv="do_suggestion_L", string="안녕하세요. 선생님 머리가 아파서 병원에 왔어요. 머리를 만져 주세요.")     
        ...
        cm.tts(bhv="do_suggestion_L", string=f"이제 파이보가 의사 선생님이 되어서 {wm.word(self.user_name, 0)}의 몸과 마음을 살펴볼게!")
        
        cm.tts(bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}의 심장은 어디있을까?")
        ...
        # 3. 대화 시작
        cm.tts(bhv="do_explain_C", string="파이보는 놀이동산에 갔을 때 마음이 가장 행복했던 것 같아.")
        cm.tts(bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}의 마음이 가장 행복했던 때는 언제였니?")
        ...
        cm.tts(bhv="do_question_S", string=f"그럼 {wm.word(self.user_name, 0)}의 마음이 제일 슬플 때는 언제였니?")
        answer = cm.responses_proc(re_bhv="do_question_L", re_q=f"{wm.word(self.user_name, 0)}의 마음이 가장 행복했던 때는 언제였니?",                                   
        ...
        cm.tts(bhv="do_explain_C", string="나는 친구들이 내 말을 들어주지 않을 때 제일 화가 났어.")
        
        cm.tts(bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}의 마음이 제일 화가 날 때는 언제였니?")
        answer = cm.responses_proc(re_bhv="do_question_L", re_q=f"{wm.word(self.user_name, 0)}의 마음이 제일 화가 날 때는 언제였니?",                                   
                                   neu_bhv="do_agree", neu="괜찮아~ 대답하기 어려울 수 있어~",
                                   neg_bhv="do_question_S", neg="무슨 일이 있었니?",
                                   act_bhv="do_question_S", act="무슨 일이 있었니?")
        
        cm.tts(bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}의 마음이 제일 콩닥콩닥 거렸을 때는 언제였니?")
        ...
        # 4. 마무리 대화
        cm.tts(bhv="do_joy_B", string=f"{wm.word(self.user_name, 0)}의 마음을 들어볼 수 있어서 좋았어. 다음에도 같이 의사 선생님 놀이 하자! 파이보가 {wm.word(self.user_name, 0)}의 몸과 마음을 살펴줄게~!")
