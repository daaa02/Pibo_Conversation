...
class Fairytale():    
    ...
    def Wolf(self):
        ...
        answer = cm.responses_proc(re_bhv="do_question_S", string=f"는 여우나 늑대를 본 적이 있다면 말해 줄래?", 
        ...
        if answer[0] == "positive" or answer[0] == "action":
            answer = cm.responses_proc(re_bhv="do_question_S", string=f"봤을 때 {wm.word(self.user_name, 0)}가 놀라진 않았니?", 
                                       neu_bhv="do_agree", neu=f"괜찮아~ 모를 수 있지~")

        cm.tts(bhv="do_question_S", string=f"늑대가 가장 좋아할 것 같은 음식을 상상해볼까?")
        answer = cm.responses_proc(re_bhv="do_question_S", string=f"늑대가 가장 좋아할 것 같은 음식을 상상해볼까?", 
        ...
        if answer[0] == "positive" or answer[0] == "action":
            answer = cm.responses_proc(re_bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}는 어떤 음식을 좋아하니?", 
        ...
        answer = cm.responses_proc(re_bhv="do_question_S", string=f"늑대가 여우를 잡아 먹는다고 했을 때 여우는 무서웠을까?", 
        ...
        if answer[0] == "positive" or answer[0] == "action":
            answer = cm.responses_proc(re_bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}는 최근에 무서웠던 적이 있다면 말해 줄래?", 
        ...
        answer = cm.responses_proc(re_bhv="do_question_S", string=f"여우가 늑대에게 무사히 도망갔을 때 마음이 편안해졌을까?", 
        ...
        if answer[0] == "positive" or answer[0] == "action":
            answer = cm.responses_proc(re_bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}도 편안한 기분을 느낀적이 있다면 말해 줄래?", 
        ...
        answer = cm.responses_proc(re_bhv="do_question_L", string=f"만약 {wm.word(self.user_name, 0)}가 음식을 아주 많이 먹은 늑대를 만난다면 뭐라고 해줄 수 있을까?", 
