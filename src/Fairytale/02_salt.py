...
class Fairytale():    
    ...
    def Salt(self):
        ...
        answer = cm.responses_proc(re_bhv="do_question_L", string=f"동화 속 소금 장수와 당나귀는 어디를 가는 길이었을까?", 
                                   neu_bhv="do_agree", neu=f"모를 수 있지~")

        cm.tts(bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}도 유치원 갈 때 가방을 매고 가니? ")
        answer = cm.responses_proc(re_bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}도 유치원 갈 때 가방을 매고 가니? ", 
                                   pos_bhv="do_question_S", pos=f"{wm.word(self.user_name, 0)} 가방 안에 어떤 것들이 들어있니?")
        
        if answer[0] == "positive":
            answer = cm.responses_proc(re_bhv="do_question_S", string=f"{wm.word(self.user_name, 0)} 가방 안에 어떤 것들이 들어있니?")
                
        cm.tts(bhv="do_question_L", string=f"{wm.word(self.user_name, 0)}가 말한 것들 말고 유치원에 가져 가고 싶은 물건이 있다면 말해줄래?")
        answer = cm.responses_proc(re_bhv="do_question_L", string=f"{wm.word(self.user_name, 0)}가 말한 것들 말고 유치원에 가져 가고 싶은 물건이 있다면 말해줄래?", 
        ...
        if answer[0] == "positive" or answer[0] == "action":
            answer = cm.responses_proc(re_bhv="do_question_S", string=f"그거 가지고 유치원에 가서 뭐하고 싶어?")            
        
        # 2. 등장인물 공감 대화
        cm.tts(bhv="do_question_L", string=f"소금 장수는 당나귀에게 속았다고 생각했을 때 소금 장수는 짜증 났을까?")
        answer = cm.responses_proc(re_bhv="do_question_L", string=f"소금 장수는 당나귀에게 속았다고 생각했을 때 소금 장수는 짜증 났을까?", 
        ...
        if answer[0] == "positive" or answer[0] == "action":
            answer = cm.responses_proc(re_bhv="do_question_L ", string=f"{wm.word(self.user_name, 0)}도 누가 {wm.word(self.user_name, 0)}를 속여서 짜증났던 적이 있다면 이야기해 줄래?", 
                                       act_bhv="do_agree", act=f"그런 일이 있었구나! 정말 짜증났었겠다! ")

        cm.tts(bhv="do_question_L", string=f"당나귀가 꾀를 부려 짐이 훨씬 더 무거워 졌을 때 당나귀는 후회했겠지?")
        answer = cm.responses_proc(re_bhv="do_question_L", string=f"당나귀가 꾀를 부려 짐이 훨씬 더 무거워 졌을 때 당나귀는 후회했겠지?", 
        ...
        if answer[0] == "positive" or answer[0] == "action":
            answer = cm.responses_proc(re_bhv="do_question_S", string=f"{wm.word(self.user_name, 0)}도 후회 했던 적이 있었다면 이야기해 줄래?", 
        ...
        answer = cm.responses_proc(re_bhv="do_suggestion_L", string=f"만약 {wm.word(self.user_name, 0)}가 게으름 피우는 당나귀를 만난다면 뭐라고 해줄 수 있을까?", 
