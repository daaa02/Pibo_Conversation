...
class Fairytale():    
    ...
    def Queenbee(self):
        ...
        cm.tts(bhv="question_S", string=f"개미들이 찾아준 천 개의 진주로 무엇을 만들었을까?")
        answer = cm.responses_proc(re_bhv="do_question_S", re_q=f"물건을 잃어버렸을 때는 어떻게 찾아야할까?", 
                                   neu_bhv="do_agree", neu=f"괜찮아~ 대답하기 어려울 수 있어~")

        cm.tts(bhv="question_S", string=f"만약 막내 왕자도 개미를 괴롭혔다면 개미들은 막내를 도와줬을까?")
        ...
        cm.tts(bhv="question_S", string=f"돌이 된 형들은 무슨 생각을 했을까?")
        answer = cm.responses_proc(re_bhv="do_question_S", re_q=f"돌이 된 형들은 무슨 생각을 했을까?", 
                                   neu_bhv="do_agree", neu=f"모를 수 있어~")  

        # 2. 등장인물 공감 대화        
        cm.tts(bhv="question_S", string=f"난쟁이에게 어려운 임무를 받았을 때 막내 왕자는 실패할까봐 걱정했겠지?")
        answer = cm.responses_proc(re_bhv="do_question_S", re_q=f"물건을 잃어버렸을 때는 어떻게 찾아야할까?", 
        ...
        cm.tts(bhv="question_S", string=f"개미랑 여왕벌이 도와주었을 때 막내 왕자는 감동 받았을까?")
