...
class Fairytale():    
    ...
    def Hermes(self):
        ...
        cm.tts(bhv="do_question_S", string=f"물건을 잃어버렸을 때는 어떻게 찾아야할까?")
        ...
        cm.tts(bhv="do_question_S", string=f"연못에는 도끼말고 어떤 물건들이 빠져있었을지 상상해볼까?")
        answer = cm.responses_proc(re_bhv="do_question_S", re_q=f"연못에는 도끼말고 어떤 물건들이 빠져있었을지 상상해볼까?", 
                                   neu_bhv="do_agree", neu=f"괜찮아~ 생각나지 않을 수 있어~")

        cm.tts(bhv="do_question_S", string=f"나무꾼은 금 도끼로 뭘 했을까?")
        answer = cm.responses_proc(re_bhv="do_question_S", re_q=f"나무꾼은 금 도끼로 뭘 했을까?")
        

        # 2. 등장인물 공감 대화
        cm.tts(bhv="do_question_S", string=f"욕심쟁이 친구가 황금도끼를 얻으려고 거짓말을 했을 때 헤르메스 신은 화가 났을까?")
        ...
        cm.tts(bhv="do_question_L", string=f"헤르메스 신이 나무꾼에게 세 가지 도끼를 상으로 주었을 때 나무꾼은 기뻤을까?")
