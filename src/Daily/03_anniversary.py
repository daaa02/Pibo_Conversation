...
class Daily():    
    ...
    def Bday(self):   
        ...
        # 1.2 대화 시작
        cm.tts(bhv="do_question_S", string="이번 생일에는 뭘 하고 싶어?")
        ...
        cm.tts(bhv="do_question_L", string=f"난 초코 케잌을 좋아하는데, {wm.word(self.user_name, 0)}는 어떤 케이크를 좋아 하니?")
        ...
        cm.tts(bhv="do_suggestion_S", string=f"파이보도 {wm.word(self.user_name, 0)} 생일 파티에 초대해줄 수 있어?")
        ...
        cm.tts(bhv="do_joy_A", string=f"행복하고 기분 좋은 생일을 보냈으면 좋겠어! {wm.word(self.user_name, 0)} 생일에 나도 특별히 생일 노래를 불러줄게! 기대해도 좋아~")     
    ...
    def Bday_Parent(self):   # 나중엔 엄마/아빠 생일 중 하나만 있어도 되게끔
        ...
        # 1.1 기념일 알림
        # audio.audio_play(filename="/home/pi/AI_pibo2/src/data/audio/**")
        cm.tts(bhv="do_joy_A", string=f"3일 뒤 {self.dob_parent[0][0]} 생일이지?")
        ...
        cm.tts(bhv="do_suggestion_L", string="영상 편지를 찍어보면 어때? 엄마 생일 축하 영상으로 말이야. 내가 동영상 찍어줄 수 있는데 지금 해볼래?")                                    
        ...
        cm.tts(bhv="do_joy_B", string="엄마 생일날 파이보와 함께 축하해드리자!")
    ...
    def ChildrensDay(self):
        ...
        # 1.1 기념일 알림
        # audio.audio_play(filename="/home/pi/AI_pibo2/src/data/audio/**")
        cm.tts(bhv="do_joy_A", string="3일 뒤 어린이날이야!")
        answer = cm.responses_proc(re_bhv="do_joy_A", re_q="3일 뒤 어린이날이야!",
                                   pos_bhv="do_joy_A", pos="어린이 날이라 기쁠 것 같아!")
        
        # 1.2 대화 시작
        cm.tts(bhv="do_question_S", string="받고 싶은 선물이 있다면 말해 볼래?")
        answer = cm.responses_proc(re_bhv="do_question_S", re_q="받고 싶은 선물이 있다면 말해 볼래?",
                                   pos_bhv="do_joy_B", pos="그걸 선물로 받으면 좋겠는 걸!",
                                   act_bhv="do_joy_B", act="그걸 선물로 받으면 좋겠는 걸!")
            
        cm.tts(bhv="do_question_S", string="이번 어린이날에 어디 가고 싶은 곳이 있니?")
        ...
        cm.tts(bhv="do_explain_A", string="어린이날은 어린이들이 씩씩하고 올바르게 자라도록 나라에서 정한 날이야! 알고 있었니?")
        ...
        cm.tts(bhv="do_joy_A", string=f"{wm.word(self.user_name, type=0)}가 정말 행복한 어린이날을 보냈으면 좋겠어!")
        
        cm.tts(bhv="do_joy_B", string="이번 어린이날에도 즐겁게 놀고 더 씩씩하고 건강한 어린이가 되자!")
    ...
    def ParentsDay(self):
        ...
        # 1.1 기념일 알림
        # audio.audio_play(filename="/home/pi/AI_pibo2/src/data/audio/**")
        cm.tts(bhv="do_joy_A", string="3일이 지나면 어버이날이네!")
        
        cm.tts(bhv="do_question_L", string=f"어버이날에는 보통 카네이션이랑 편지를 드린다고 하던데, {wm.word(self.user_name, type=0)}도 드려본 적 있어?")
        ...
        cm.tts(bhv="do_explain_A", string="어버이날은 나를 낳아주신 부모님과 할머니, 할아버지께 감사하는 날이야. 알고 있었니?")
        answer = cm.responses_proc(re_bhv="do_explain_A", re_q="어버이날은 나를 낳아주신 부모님과 할머니, 할아버지께 감사하는 날이야. 알고 있었니?",
                                   pos_bhv="do_compliment_S", pos="정말 대단한 걸?",
                                   neu_bhv="do_agree", neu="괜찮아. 파이보가 알려줬잖아~",
                                   act_bhv="do_agree", act="괜찮아. 파이보가 알려줬잖아~")
                                   
        cm.tts(bhv="do_suggestion_L", string="이번 어버이날에는 부모님께 어버이날 노래도 불러드리면 어떨까? 내가 동영상 찍어 줄 수 있어!")                                    
        answer = cm.responses_proc(re_bhv="do_suggestion_S", re_q="내가 동영상 찍어줄 수 있는데 지금 해볼래?",
                                   pos_bhv="do_waiting_A", pos="좋아! 준비가 되면 준비 됐다고 말해줘~",
                                   neu_bhv="do_explain_B", neu="그럼 다음에 찍어 드리자!",
                                   neg_bhv="do_explain_B", neg="그럼 다음에 찍어 드리자!")
        ...
        cm.tts(bhv="do_joy_B", string="어버이날에 내가 가족사진도 멋지게 찍어줄게! 기대해~")
    ...
    def Christmas(self):
        ...
        # 1.1 기념일 알림
        # audio.audio_play(filename="/home/pi/AI_pibo2/src/data/audio/**")
        cm.tts(bhv="do_joy_A", string="3밤만 자면 크리스마스야!")
        
        cm.tts(bhv="do_question_L", string=f"{wm.word(self.user_name, type=0)}가 좋아하는 크리스마스 캐롤이 뭔지 말해줄래?")
        ...
        cm.tts(bhv="do_question_S", string="최근에 멋진 크리스마스 트리를 본 적이 있니?")
        ...
        cm.tts(bhv="do_question_S", string="이번 크리스마스에 멋진 계획이 있다면 말해 줄래?")
        ...
        cm.tts(bhv="do_question_S", string="크리스마스에 받고 싶은 선물이 있다면 알려줄래?")
        answer = cm.responses_proc(re_bhv="do_question_S", re_q="크리스마스에 받고 싶은 선물이 있다면 알려줄래?")
        
        cm.tts(bhv="do_joy_A", string=f"{wm.word(self.user_name, type=0)}가 행복하고 멋진 크리스마스를 보냈으면 좋겠어!")

        cm.tts(bhv="do_joy_B", string="크리스마스에 내가 멋진 캐롤을 들려줄게~ 기대해!")
